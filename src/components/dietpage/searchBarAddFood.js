const foodAdder = (
  ...
  const selectedFoodMeasureInGrams =
    selectedFoodMeasureDetails.serving_weight / selectedFoodMeasureDetails.qty;
  ...
  const consumedNutrientValues = nutrients.map(nutrients => {
    const nutrientsPerGram = nutrients / copyOfFoodDetails.serving_weight_grams;
  ...
  for (let i = 0; i < nutrientsPerGramArr.length; i++) {
    nutrientsNamesAndValuesPerGram[nutrientNames[i]] = nutrientsPerGramArr[i];
  }
